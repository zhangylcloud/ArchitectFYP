...
public class GrassEditor : MaterialEditor
{
	...
	private static readonly string[] defaultKeywords = { "SIMPLE_GRASS", "GRASS_HEIGHT_SMOOTHING", "GRASS_CALC_GLOBAL_WIND" };
	...
	public override void OnInspectorGUI()
	{
		...
		bool grassRandomDirection = originalKeywords.Contains("GRASS_RANDOM_DIR");
		...
		grassRandomDirection = GUILayout.Toggle(grassRandomDirection, "Randomize grass orientation");
		...
		calcGlobalWind = GUILayout.Toggle(calcGlobalWind, "Use global wind");
		EditorGUILayout.Separator();
			...
			foreach (var property in properties)
			{
				...
				if (property.name == "_MaxTessellation")
				{
					property.floatValue = EditorGUILayout.IntSlider(property.displayName, (int)property.floatValue,
						(int)property.rangeLimits.x, (int)property.rangeLimits.y);
					continue;
				}

				//Max tessellation can only be a int between 1 and 6
				if (property.name == "_LODMax")
				{
					property.floatValue = EditorGUILayout.IntSlider(property.displayName, (int)property.floatValue,
						(int)property.rangeLimits.x, (int) property.rangeLimits.y);
					continue;
				...
				{
					EditorGUI.BeginDisabledGroup(!calcGlobalWind);
